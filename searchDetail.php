...
<mainRight id="mainRight" class="md:w-3/5 bg-pink-300">
  ...
  <div class="text-center my-4">
        <div class="text-blue-500 mb text-2xl">Details of selected data</div>
        <div class="text-blue-500">Editing is possible as long as it is above TL.</div>
        <div class="text-blue-500 mb-2">Click on the Edit button that appears in the lower right corner.</div>
        <div class="text-red-700 text-sm mb-2">Clicking on the <span class="ml-2 fa-brands fa-google"></span> (google button) will automatically perform a google searchfor that word.</div>
        <!-- Formタグ -->
            ...
            <div name="edit">
              ...
              <div class="my-1">
                  <div class="flex justify-center">
                      ...
                      <div>InjVol：<input class="bg-slate-300 mb-1 px-1 rounded w-2/3" type="text" name="InjVol" value="<?=$row['InjVol']?>" placeholder="Preferably input" readonly></div>
                  </div>
                  <div>SamplePrep：<input class="bg-slate-300 mb-1 px-1 rounded w-3/5" type="text" name="SamplePrep" value="<?=$row['SamplePrep']?>" placeholder="Preferably input" readonly></div>
              </div>
              <div class="my-1">
                  <div>Column：<input class="bg-slate-300 mb-1 px-1 rounded w-3/5" type="text" name="ColumnName" value="<?=$row['ColumnName']?>" placeholder="Preferably input" readonly><a href="https://www.google.com/search?q=<?=$row['ColumnName']?>" target="_blank"><span class="ml-2 fa-brands fa-google"></span></a></div>
                  <div class="flex justify-center">
                      ...
                      <div>Col-Temp(℃)：<input class="bg-slate-300 mb-1 px-1 rounded w-2/5" type="text" name="ColTemp" value="<?=$row['ColTemp']?>" placeholder="Preferably input" readonly></div>
                  </div>
              </div>
              <div class="my-1">
                  <div>EluentA：<input class="bg-slate-300 mb-1 px-1 rounded w-3/5" type="text" name="MPA" value="<?=$row['MPA']?>" placeholder="Empty OK" readonly></div>
                  <div>EluentB：<input class="bg-slate-300 mb-1 px-1 rounded w-3/5" type="text" name="MPB" value="<?=$row['MPB']?>" placeholder="Empty OK" readonly></div>
                  <div>EluentC：<input class="bg-slate-300 mb-1 px-1 rounded w-3/5" type="text" name="MPC" value="<?=$row['MPC']?>" placeholder="Empty OK" readonly></div>
                  <div>EluentD：<input class="bg-slate-300 mb-1 px-1 rounded w-3/5" type="text" name="MPD" value="<?=$row['MPD']?>" placeholder="Empty OK" readonly></div>
                  <div>TimeProg：<input class="bg-slate-300 mb-1 px-1 rounded w-4/5" type="text" name="TimeProg" value="<?=$row['TimeProg']?>" required readonly></div>
              </div>
              <div class="my-1">
                  <div>Target RT(min)：<input class="bg-slate-300 mb-1 px-1 rounded w-2/5" type="text" name="TargetTime" value="<?=$row['TargetTime']?>" placeholder="Necessary" required readonly></div>
                  <div class="flex justify-center">
                      <div>RawMat RT：<input class="bg-slate-300 mb-1 px-1 rounded w-2/5" type="text" name="RawMatTime" value="<?=$row['RawMatTime']?>" placeholder="Empty OK" readonly></div>
                      <div>Impurity RT：<input class="bg-slate-300 mb-1 px-1 rounded w-2/5" type="text" name="ImpurityTime" value="<?=$row['ImpurityTime']?>" placeholder="Empty OK" readonly></div>
                  </div>
              </div>
              <div class="my-1">
              ...
              <label>Comment：<br><textarea class="bg-slate-300 mb-1 px-1 rounded-md w-full" name="Comment" rows="4" cols="40" placeholder="Please fill in any incomplete, omitted, misspelled words, special notes, etc. in the form up to this point." readonly><?=$row['Comment']?></textarea></label><br>
                 ...
                 <div class="flex justify-end">
                  <?php if($_SESSION['kanri_flg'] == 1 OR $_SESSION['kanri_flg'] == 2):?><a class="rounded-lg border-2 border-gray-400 py-1 px-4 bg-gray-100 hover:bg-pink-500" href="searchDetailEdit.php?id=<?=$row["id"]?>">Edit!</a><?php endif;?>
                 </div>
